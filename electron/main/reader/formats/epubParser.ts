/**
 * EPUB 纯解析实现（无 electron 依赖）。
 *
 * 该模块同时被两端引用：
 * - 主进程：作为 parser worker 挂掉后的内联降级路径（见 workers/parserHost.ts）
 * - parser utilityProcess worker：workers/parser-worker.ts 直接调用
 *
 * 因此这里只允许依赖 node 内置模块、node-stream-zip、jsdom 与纯工具模块，
 * 严禁 import electron 或任何会传递引入 electron 的业务模块。
 */
...
import type { ChapterContent, ParsedBook } from "./types";
...
/**
 * 在已解析的章节 DOM 上就地内联图片资源并剥离外链 CSS / 脚本。
 * 注意：会修改传入的 document（调用方需先取完原始 textContent / 标题）。
 */
...
/** 解析整本 EPUB：元数据 + TOC + 全文 + 封面。 */
export async function parseEpub(absolutePath: string): Promise<ParsedBook> {
...
/** 取单章渲染内容（同一章 HTML 只做一次 JSDOM 解析）。 */
export async function getEpubChapter(
	...
	try {
		...
		// 同一章只 parse 一次：先在原始 DOM 上取文本与标题，
		// 再让 rewriteResources 就地改写同一个 document。
		const dom = new JSDOM(rawHtml, { contentType: "application/xhtml+xml" });
		const doc = dom.window.document;
		const text = doc.body?.textContent || "";
		const titleFromDoc = (
			doc.querySelector("h1, h2, title")?.textContent || ""
		).trim();
		const { html, images } = await rewriteResources(doc, zip, chapterDir);
