    ...
    async backupToWebdav(
        ...
        try {
            ...
            if (webdavConfig.encryptionPassword) {
                ...
                fileContent = CryptoService.encryptFile(rawContent, webdavConfig.encryptionPassword);
                console.log(`[BackupManager] Encrypted: ${rawContent.length} -> ${fileContent.length} bytes`);
            } else {
    ...
    async restoreFromWebdav(
        ...
        try {
            ...
            if (webdavConfig.encryptionPassword) {
                ...
                fileBuffer = CryptoService.decryptFile(encryptedBuffer, webdavConfig.encryptionPassword);
                console.log(`[BackupManager] Decrypted: ${encryptedBuffer.length} -> ${fileBuffer.length} bytes`);
            } else {
