...
import { createHarnessBridgeHandlers } from "./handlers/harnessBridge";
import { createHarnessAutomationHandlers } from "./handlers/harnessAutomation";
...
export function registerIpcHandlers({
	...
	// Harness Bridge handlers (互为工具 / 议会 / 共享白板 / 路由 / 额度 / 反向 MCP)
	const harnessBridgeHandlers = createHarnessBridgeHandlers(db, {
		getMainWindow: () => mainWindowRef,
	});

	// Harness 自动化 handlers (运行态监测 / 定时任务 / 失败重试守护)
	const harnessAutomationHandlers = createHarnessAutomationHandlers(
		db,
		() => mainWindowRef,
	);
	...
	handle("harness_usage_stats", harnessHubHandlers.harness_usage_stats);
	...
	handle("harness_handoff_plan", harnessHubHandlers.harness_handoff_plan);
	...
	handle("harness_resume_launch", harnessHubHandlers.harness_resume_launch);
	...
	handle("harness_session_export", harnessHubHandlers.harness_session_export);
	handle("harness_session_import", harnessHubHandlers.harness_session_import);
	handle(
		"harness_import_candidates",
		harnessHubHandlers.harness_import_candidates,
	);

	// ==================
	// AI Hub 互通升级（互为工具 / 议会 / 白板 / 路由 / 额度 / 反向 MCP）
	// ==================
	handle("harness_bridge_call", harnessBridgeHandlers.harness_bridge_call);
	handle(
		"harness_bridge_calls_list",
		harnessBridgeHandlers.harness_bridge_calls_list,
	);
	handle("harness_mcp_status", harnessBridgeHandlers.harness_mcp_status);
	handle(
		"harness_mcp_set_enabled",
		harnessBridgeHandlers.harness_mcp_set_enabled,
	);
	handle(
		"harness_mcp_rotate_token",
		harnessBridgeHandlers.harness_mcp_rotate_token,
	);
	handle("harness_council_run", harnessBridgeHandlers.harness_council_run);
	handle("harness_council_list", harnessBridgeHandlers.harness_council_list);
	handle("harness_council_get", harnessBridgeHandlers.harness_council_get);
	handle("harness_board_list", harnessBridgeHandlers.harness_board_list);
	handle("harness_board_add", harnessBridgeHandlers.harness_board_add);
	handle("harness_board_update", harnessBridgeHandlers.harness_board_update);
	handle("harness_board_remove", harnessBridgeHandlers.harness_board_remove);
	handle("harness_routes_list", harnessBridgeHandlers.harness_routes_list);
	handle("harness_route_save", harnessBridgeHandlers.harness_route_save);
	handle("harness_route_reset", harnessBridgeHandlers.harness_route_reset);
	handle("harness_route_resolve", harnessBridgeHandlers.harness_route_resolve);
	handle("harness_quota_list", harnessBridgeHandlers.harness_quota_list);
	handle("harness_quota_refresh", harnessBridgeHandlers.harness_quota_refresh);
	handle(
		"harness_quota_set_block",
		harnessBridgeHandlers.harness_quota_set_block,
	);
	handle("harness_quota_clear", harnessBridgeHandlers.harness_quota_clear);
	handle("harness_settings_get", harnessBridgeHandlers.harness_settings_get);
	handle("harness_settings_save", harnessBridgeHandlers.harness_settings_save);

	// ==================
	// Harness 自动化（运行态监测 / 定时任务 / 失败重试守护）
	// ==================
	handle(
		"harness_runtime_list",
		harnessAutomationHandlers.harness_runtime_list,
	);
	handle(
		"harness_runtime_abort",
		harnessAutomationHandlers.harness_runtime_abort,
	);
	handle("harness_job_list", harnessAutomationHandlers.harness_job_list);
	handle("harness_job_save", harnessAutomationHandlers.harness_job_save);
	handle("harness_job_delete", harnessAutomationHandlers.harness_job_delete);
	handle(
		"harness_job_set_enabled",
		harnessAutomationHandlers.harness_job_set_enabled,
	);
	handle("harness_job_run_now", harnessAutomationHandlers.harness_job_run_now);
	handle("harness_job_cancel", harnessAutomationHandlers.harness_job_cancel);
	handle(
		"harness_job_runs_list",
		harnessAutomationHandlers.harness_job_runs_list,
	);
	handle("harness_job_run_get", harnessAutomationHandlers.harness_job_run_get);
	...
	handle("aihub_send_session", harnessHubHandlers.aihub_send_session);
	handle("aihub_cookie_sources", harnessHubHandlers.aihub_cookie_sources);
	handle("aihub_import_cookies", harnessHubHandlers.aihub_import_cookies);
	handle("aihub_reload", harnessHubHandlers.aihub_reload);
	...
	handle(
		...
		importExportHandlers.export_all_data_to_file,
	);
