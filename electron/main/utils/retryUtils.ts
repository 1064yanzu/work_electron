...
export async function withRetry<T>(
	...
	for (let attempt = 0; attempt <= config.maxRetries; attempt++) {
		...
		if (attempt > 0 && lastError) {
			...
			options?.logger?.(
				`[Retry ${attempt}/${config.maxRetries}] ${lastError.message}, waiting ${Math.round(delayMs)}ms...`,
			);
		...
		try {
			...
			if (!isRetryableError(lastError)) {
				options?.logger?.(
					`[No Retry] Non-retryable error: ${lastError.message}`,
				);
				throw lastError;
			}
			...
			if (attempt >= config.maxRetries) {
				options?.logger?.(
					`[Retry Failed] Max retries reached, giving up: ${lastError.message}`,
				);
				throw lastError;
			}
			...
			// 否则继续重试
			options?.logger?.(
				`[Will Retry] Retryable error detected: ${lastError.message}`,
			);
		}
	}
