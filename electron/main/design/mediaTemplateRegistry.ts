...
function getBuiltinPromptTemplatesRoot(): string {
  return path.join(getDesignLibraryRoot(), "vendor", "open-design", "prompt-templates");
}
...
function toSummary(
  ...
  if (source === "builtin" && raw.id) {
    const localPreviewPng = path.join(path.dirname(filePath), `${raw.id}.preview.png`);
    const localPreviewJpg = path.join(path.dirname(filePath), `${raw.id}.preview.jpg`);
    if (!resolvedPreview) {
...
async function scanDir(
  ...
  for (const ent of entries) {
    ...
    result.push({ summary: toSummary(raw, source, detectedKind, filePath), filePath });
  }
  return result;
}
...
export async function listMediaTemplateSummaries(
  ...
  const kinds: MediaTemplateKind[] =
    input?.kind && (["image", "video", "audio"] as MediaTemplateKind[]).includes(input.kind)
      ? [input.kind]
      : ["image", "video", "audio"];
...
export async function getMediaTemplate(
  ...
  if (!source || source === "builtin") {
    candidates.push({ root: getBuiltinPromptTemplatesRoot(), source: "builtin" });
  }
  ...
  for (const cand of candidates) {
    for (const kind of ["image", "video", "audio"] as MediaTemplateKind[]) {
      ...
      return {
        ...
        source_meta: raw.source && typeof raw.source === "object"
...
export async function saveMediaTemplate(
  ...
  const id = safeFileId(input.id || `${input.kind}-${randomUUID().slice(0, 8)}`);
  const now = Date.now();
...
export async function importMediaTemplateFromFile(
  ...
  if (!detectedKind) {
    throw new Error("模板缺少有效的 surface/kind 字段（应为 image / video / audio）");
  }
...
export async function deleteUserMediaTemplate(
  ...
  const safeId = safeFileId(id);
  const filePath = path.join(getUserMediaTemplatesRoot(), kind, `${safeId}.json`);
  try {
