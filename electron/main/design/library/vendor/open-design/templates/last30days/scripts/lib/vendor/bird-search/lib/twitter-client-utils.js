...
export function renderContentState(contentState) {
	...
	for (const block of contentState.blocks) {
		...
		if (
			...
			previousBlockType === "ordered-list-item"
		) {
	...
	for (const range of linkRanges) {
		...
		if (entity?.data.url) {
			...
			text =
				...
				markdownLink +
				text.slice(range.offset + range.length);
		}
	}
	return text.trim();
}
...
export function extractMedia(result) {
	...
	const rawMedia =
		result?.legacy?.extended_entities?.media ?? result?.legacy?.entities?.media;
...
export function mapTweetResult(result, quoteDepthOrOptions) {
	const options =
		typeof quoteDepthOrOptions === "number"
...
export function parseUsersFromInstructions(instructions) {
	...
	for (const instruction of instructions) {
		...
		for (const entry of instruction.entries) {
			...
			const userResult =
				rawUserResult?.__typename === "UserWithVisibilityResults" &&
				rawUserResult.user
					? rawUserResult.user
					: rawUserResult;
