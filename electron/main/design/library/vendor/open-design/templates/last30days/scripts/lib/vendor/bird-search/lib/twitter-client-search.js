	...
	try {
		...
		return (
			parsed.errors?.some((error) => {
				...
				if (
					error?.path?.includes("rawQuery") &&
					RAW_QUERY_MISSING_REGEX.test(error.message ?? "")
				) {
					return true;
				}
				return false;
	...
	class TwitterClientSearch extends Base {
		...
		async searchPaged(query, limit, options = {}) {
					...
					try {
						...
						const instructions =
							data.data?.search_by_raw_query?.search_timeline?.timeline
								?.instructions;
						const pageTweets = parseTweetsFromInstructions(instructions, {
			...
			while (unlimited || tweets.length < limit) {
				const pageCount = unlimited
					? pageSize
					: Math.min(pageSize, limit - tweets.length);
