...
export async function resolveCredentials(options) {
	...
	if (disableBrowserCookies) {
		if (!cookies.authToken) {
			warnings.push(
				"Missing auth_token - provide via --auth-token, AUTH_TOKEN env var, or disable BIRD_DISABLE_BROWSER_COOKIES to allow browser cookie lookup",
			);
		}
		if (!cookies.ct0) {
			warnings.push(
				"Missing ct0 - provide via --ct0, CT0 env var, or disable BIRD_DISABLE_BROWSER_COOKIES to allow browser cookie lookup",
			);
		}
		return { cookies, warnings };
	}
	...
	try {
		...
		warnings.push(
			"Browser cookie lookup unavailable because vendored dependency @steipete/sweet-cookie is not installed.",
		);
		if (!cookies.authToken) {
			warnings.push(
