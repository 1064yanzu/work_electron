...
function toFeatureOverrides(overrides) {
	...
	const setEntries = Object.entries(overrides.sets).filter(
...
function resolveFeaturesCachePath() {
	const override =
		process.env.BIRD_FEATURES_CACHE ?? process.env.BIRD_FEATURES_PATH;
...
export function applyFeatureOverrides(setName, base) {
	...
	if (
		...
		(!setOverrides || Object.keys(setOverrides).length === 0)
	) {
		return base;
	}
	if (setOverrides) {
