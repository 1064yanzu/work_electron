...
9. **（默认）导出视频 · 必带音频**：动画 HTML 的**默认交付形态是带音频的 MP4**，不是纯画面。无声版本等于半成品——用户潜意识感知「画在动但没声音响应」，廉价感的根源就在这里。流水线：
   - `scripts/render-video.js` 录 25fps 纯画面 MP4（只是中间产物，**不是成品**）
   - `scripts/convert-formats.sh` 派生 60fps MP4 + palette 优化 GIF（视平台需要）
   - **有旁白需求时**：先用 `scripts/generate-tts.js` 按对应 provider 的配置文件生成 TTS 音轨（当前已兼容 Xiaomi MiMo TTS、GeekAI、NewAPI/OpenAI 兼容 `audio/speech`）
   - `scripts/add-music.sh` 合成最终成品音轨：支持 `BGM only`、`voiceover only`、`voiceover + BGM` 三种模式
   ...
   - **有讲解场景时优先考虑 TTS / 真人旁白**。教程、解释型信息图、产品 walkthrough，只给 BGM 不给 voiceover，信息传达会断层
   - **BGM + SFX 双轨制必须同时做**——只做 BGM 是 ⅓ 分完成度；若还存在旁白，则旁白优先、BGM 负责铺底，频段隔离与 ducking 见 `references/audio-design-rules.md`
   - 交付前 `ffprobe -select_streams a` 确认有 audio stream，没有则不是成品
   - **跳过音频的条件**：用户明确说「不要音频」「纯画面」「我要自己配音」——否则默认带。
   - 参考完整流程见 `references/video-export.md` + `references/audio-design-rules.md` + `references/sfx-library.md` + `docs/mimo-tts.md`。
...
| `view_transitions.js` + `deck_transitions.css` | **deck 转场（可选 opt-in）** | 6 种 view-transition：fade（默认）/ push / cover / morph / cinematic-cut / keynote-magic-move。浏览器原生 + 旧版 fallback 同步执行。详见 `references/slide-decks.md` §「转场选择决策树」 |
...
| `animations.jsx` | 任何动画HTML | Stage + Sprite + useTime + Easing + interpolate · `chrome="minimal" | "studio" | "none"`（v2 起） |
| `stage_chrome.jsx` | **调试 / 演示精细动画时** · `<Stage chrome="studio">` | frame-step（J/K/L + ←/→） + rate（0.25/0.5/1/2×）+ 区段 loop + chapter markers + speaker note 抽屉 + REC 徽章 + 截图。详见 `references/stage-chrome.md` |
| `motion_pack/gsap_adapter.jsx` + `text_split.jsx` + `lottie_player.jsx` + `auto_animate.jsx` | **超出自研 engine 范围的动效**（按字符 stagger / SVG morph / Lottie / Layout 自动 morph） | useGsap 把 GSAP timeline 绑死到 Stage 时间，scrub / 录视频都同步。详见 `references/gsap-recipes.md` |
| `sfx_runtime.js` + `sfx_cue.jsx` + `sfx/INDEX.json` | **HTML 实时演示 SFX**（不再静音） | `<SFXCue at={2.3} src="ui/click" />` 声明式 cue；录视频时自动转写 `window.__sfxCues` 给 `scripts/extract-sfx-timeline.js` 提取，消除人工算 ms |
| `tts_player.jsx` | **HTML 内同步播放 TTS 旁白** | `<TTSPlayer src vtt />` Stage 时间 ↔ audio.currentTime 双向锁；录视频时自动隐藏（音轨由 add-music.sh 后期注入） |
...
| 做动画/motion（**先读 pitfalls**）| `references/animation-pitfalls.md` + `references/animations.md` + `assets/animations.jsx` · **高级动效** → `assets/motion_pack/*` + `references/gsap-recipes.md` · **调试时启用** → `chrome="studio"` 见 `references/stage-chrome.md` |
...
| **动画加 TTS / 旁白** | `docs/mimo-tts.md` + `docs/examples/*.config.example.json` + `scripts/generate-tts.js` + `scripts/add-music.sh` · **零成本兜底** → `provider: edge-tts / piper / web-speech` · **字幕** → `--subtitles` 配 `--burn-subtitles` · **时长预配平** → `scripts/estimate-tts-duration.js` · **多角色对话** → `dialogue` schema · **HTML 内播** → `assets/tts_player.jsx` |
| **动画加音效SFX**（苹果发布会级，37个预制） | `references/sfx-library.md` + `assets/sfx/<category>/*.mp3` · **HTML 实时预览** → `assets/sfx_runtime.js` + `assets/sfx_cue.jsx` · **自动 cue 时间轴** → `scripts/extract-sfx-timeline.js` + `add-music.sh --sfx-cues=…` |
| **deck 转场 / view-transitions** | `assets/view_transitions.js` + `assets/deck_transitions.css` · 决策树 → `references/slide-decks.md` §「转场选择决策树」 |
| **依赖与许可证清单** | `references/dependencies-license.md` |
