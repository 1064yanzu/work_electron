...
export const WEBDAV_PROVIDERS: WebDavProvider[] = [
	...
	},
	{
		...
		defaultPath: "/workbench-sync",
		helpUrl:
			"https://doc.owncloud.com/server/user_manual/files/access_webdav.html",
	...
	},
	{
		...
		defaultPath: "/workbench-sync",
		helpUrl:
			"https://www.infomaniak.com/en/support/faq/2399/kdrive-accessing-kdrive-using-webdav",
...
export function guessProviderByHost(host: string): WebDavProvider | undefined {
	...
	if (
		normalizedHost.includes("teracloud") ||
		normalizedHost.includes("cloud.jp")
	) {
 ...
 * 验证 WebDAV URL 格式
 */
export function validateWebdavUrl(url: string): {
	valid: boolean;
	message?: string;
} {
	if (!url) {
