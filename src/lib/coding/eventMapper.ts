...
export function processUIEvent(event: UIEvent): void {
	switch (event.type) {
		...
		case 'thought_delta': {
			codingSessionStore.appendThinking(event.content, event.title || '思考过程');
			break;
		}
		...
		case 'tool_call_end': {
			const updates: Partial<import('../stores/codingSessionTypes').SessionToolCall> = {
				output: event.output,
			...
			if (toolCall && FILE_WRITE_TOOLS.has(toolCall.name)) {
				...
				if (filePath) {
					...
					const newContent = (input.new_string as string) || (input.content as string) || '';
					...
					if (activeThreadId) {
						...
						if (thread) {
							const stats = parseDiffStats(generateDiff(oldContent, newContent));
							codingThreadStore.updateThread(activeThreadId, {
								diffStats: {
									additions: (thread.diffStats?.additions ?? 0) + stats.additions,
									deletions: (thread.diffStats?.deletions ?? 0) + stats.deletions,
								},
							});
						}
					}
