...
// 默认提示词模板
const DEFAULT_PROMPTS: Omit<CustomPrompt, "id" | "createdAt" | "updatedAt" | "folderId">[] = [
    {
        name: "代码审查",
...
class CustomPromptStore {
    ...
    private async loadFromStorage() {
        try {
            ...
            if (stored) {
                const data = JSON.parse(stored) as { prompts: CustomPrompt[]; folders: PromptFolder[] };
                this.setState((state) => ({
                    ...state,
            ...
            if (legacyStored) {
                const legacyPrompts = JSON.parse(legacyStored) as Array<CustomPrompt & { category?: string }>;
                ...
                const now = Date.now();
                const folders: PromptFolder[] = Array.from(categorySet).map((name, i) => ({
                ...
                folders.forEach(f => { categoryToFolderId[f.name] = f.id; });
            ...
            const prompts = DEFAULT_PROMPTS.map((p, index) => ({
                ...p,
                id: `default-${index}`,
                folderId: index === 0 ? folderMap["开发"] : index === 1 ? folderMap["写作"] : folderMap["翻译"],
                createdAt: now,
                updatedAt: now,
            }));
    ...
    // 更新文件夹
    updateFolder(id: string, updates: Partial<Omit<PromptFolder, "id" | "createdAt">>) {
        this.setState((state) => ({
            ...state,
    ...
    deleteFolder(id: string, deletePrompts: boolean = false) {
        ...
        this.saveToStorage();
        console.log("[CustomPromptStore] 删除文件夹:", id, deletePrompts ? "(含提示词)" : "(保留提示词)");
    }
    ...
    movePromptToFolder(promptId: string, folderId: string | undefined) {
        ...
        this.saveToStorage();
        console.log("[CustomPromptStore] 移动提示词:", promptId, "->", folderId || "未分类");
    }
    ...
    // 更新提示词
    updatePrompt(id: string, updates: Partial<Omit<CustomPrompt, "id" | "createdAt">>) {
        this.setState((state) => ({
            ...state,
            prompts: state.prompts.map((p) =>
    ...
    // 导入提示词
    importPrompts(jsonStr: string): { success: boolean; count: number; error?: string } {
        try {
...
export function useCustomPromptStore() {
    ...
    return {
        ...
        movePromptToFolder: customPromptStore.movePromptToFolder.bind(customPromptStore),
        // 提示词操作
        ...
        getPromptsByFolder: customPromptStore.getPromptsByFolder.bind(customPromptStore),
        // 兼容旧 API
        getCategories: customPromptStore.getCategories.bind(customPromptStore),
        getPromptsByCategory: customPromptStore.getPromptsByCategory.bind(customPromptStore),
        // 导入导出
