...
// ━━━ 0. B.AI 极简（新默认 — 奶油暖白分层 + 赤陶签名色） ━━━
// 层次策略：侧栏坐在更深的奶油底（--t-bg）上，中栏与卡片浮在暖白（--t-bg-surface）上，
// 三栏靠底色深浅天然分层，而不是靠发丝线硬切。primary 用项目签名赤陶橙
//（与 CLAUDE.md「主色 #D96C46 赤陶橙」及 classic 主题同族），黑色单色版曾让
// 整个界面没有任何品牌色存在感，被用户否决。
const bai: ThemeDefinition = {
	id: "bai",
	name: "暖调极简",
	description: "奶油暖白分层底色，赤陶橙签名色 + 黑色正文",
	preview: { bg: "#F2F0E9", accent: "#C96442", text: "#1A1A19" },
	light: {
		"--t-bg": "#F2F0E9",
		"--t-bg-surface": "#FCFBF8",
		"--t-bg-muted": "#EBE8DE",
		"--t-bg-panel": "rgba(242,240,233,0.82)",
		"--t-bg-panel-strong": "rgba(252,251,248,0.92)",
		"--t-border": "#E2DED2",
		"--t-border-subtle": "rgba(26,26,25,0.07)",
		"--t-primary": "#C96442",
		"--t-primary-hover": "#B5573A",
		"--t-primary-muted": "rgba(201,100,66,0.10)",
		"--t-primary-fg": "#FFFFFF",
		"--t-text-primary": "#1A1A19",
		"--t-text-secondary": "#6B6B68",
		"--t-text-muted": "#9D9D98",
		"--t-ring": "rgba(201,100,66,0.30)",
		"--t-scrollbar": "#DFDBCF",
		"--t-scrollbar-hover": "#CFCABB",
	...
	dark: {
		...
		"--t-bg-panel-strong": "rgba(35,35,33,0.86)",
		"--t-border": "#32322F",
		"--t-border-subtle": "rgba(255,255,255,0.08)",
		"--t-primary": "#D97757",
		"--t-primary-hover": "#E08862",
		"--t-primary-muted": "rgba(217,119,87,0.14)",
		"--t-primary-fg": "#FFFFFF",
		"--t-text-primary": "#FAF9F5",
		"--t-text-secondary": "#B5B3AC",
		"--t-text-muted": "#7E7C76",
		"--t-ring": "rgba(217,119,87,0.40)",
		"--t-scrollbar": "#2F2F2C",
...
const classic: ThemeDefinition = {
	...
	light: {
		...
		"--t-border": "#e8e6dc",
		"--t-border-subtle": "rgba(0,0,0,0.07)",
		"--t-primary": "#c96442",
		"--t-primary-hover": "#b5573a",
...
const ocean: ThemeDefinition = {
	...
	light: {
		...
		"--t-warning": "#d97706",
		"--t-warning-muted": "rgba(217,119,6,0.12)",
		/* info 不能与 primary 同值，否则「信息提示」与「主按钮/选中态」无法区分 */
		"--t-info": "#0284c7",
		"--t-info-muted": "rgba(2,132,199,0.12)",
	},
	dark: {
		"--t-bg": "#0a1629",
		"--t-bg-surface": "#132238",
		...
		"--t-warning": "#fbbf24",
		"--t-warning-muted": "rgba(251,191,36,0.15)",
		/* 与 primary（蓝）拉开色相，偏青一档 */
		"--t-info": "#38bdf8",
		"--t-info-muted": "rgba(56,189,248,0.15)",
	},
};
...
const forest: ThemeDefinition = {
	...
	light: {
		...
		/* success 不能与 primary（翠绿）同值，偏青让「完成态」可辨 */
		"--t-success": "#0d9488",
		"--t-success-muted": "rgba(13,148,136,0.12)",
		"--t-error": "#dc2626",
		"--t-error-muted": "rgba(220,38,38,0.12)",
		"--t-warning": "#d97706",
		"--t-warning-muted": "rgba(217,119,6,0.12)",
	...
	dark: {
		...
		/* 暗色 success 与 primary-hover 撞色（#4ade80），偏青拉开 */
		"--t-success": "#2dd4bf",
		"--t-success-muted": "rgba(45,212,191,0.15)",
		"--t-error": "#f87171",
		"--t-error-muted": "rgba(248,113,113,0.15)",
		"--t-warning": "#fbbf24",
		"--t-warning-muted": "rgba(251,191,36,0.15)",
...
const midnight: ThemeDefinition = {
	...
	dark: {
		...
		/* 三级文字承载时间戳/说明等真实内容，#52525b 在 #09090b 上仅 ~2.5:1，提到 ~4:1 */
		"--t-text-muted": "#71717a",
...
const amber: ThemeDefinition = {
	...
	light: {
		...
		"--t-error": "#dc2626",
		"--t-error-muted": "rgba(220,38,38,0.12)",
		/* warning 不能与 primary（琥珀金）同值，偏橙红让「警告」可辨 */
		"--t-warning": "#c2410c",
		"--t-warning-muted": "rgba(194,65,12,0.12)",
		"--t-info": "#2563eb",
		"--t-info-muted": "rgba(37,99,235,0.12)",
	},
	dark: {
		...
		"--t-error": "#f87171",
		"--t-error-muted": "rgba(248,113,113,0.15)",
		/* 暗色 warning 与 primary 撞色（#fbbf24），偏橙拉开 */
		"--t-warning": "#fb923c",
		"--t-warning-muted": "rgba(251,146,60,0.15)",
		"--t-info": "#60a5fa",
		"--t-info-muted": "rgba(96,165,250,0.15)",
	},
};
...
const glass: ThemeDefinition = {
	...
	dark: {
		...
		/* 暗色玻璃底上 #636366 对比不足，对齐 light 版取值保证可读 */
		"--t-text-muted": "#8e8e93",
