...
export function inferScenarioFromTask(taskDescription: string): AgentScenario {
    ...
    // 快速搜索相关
    if (lower.includes('查找') || lower.includes('搜索') || lower.includes('find') || lower.includes('search')) {
        return 'fast_search';
    }
    ...
    // 深度分析相关
    if (lower.includes('分析') || lower.includes('架构') || lower.includes('重构') || lower.includes('analyze')) {
        return 'deep_analysis';
    }

    // 写作相关
    if (lower.includes('写') || lower.includes('文章') || lower.includes('润色') || lower.includes('write')) {
        return 'writing';
    }
    ...
    // 调试相关
    if (lower.includes('debug') || lower.includes('调试') || lower.includes('bug') || lower.includes('错误')) {
        return 'debugging';
    }

    // 数据处理相关
    if (lower.includes('提取') || lower.includes('转换') || lower.includes('extract') || lower.includes('convert')) {
 ...
 * 生成子代理配置(基于用户的场景配置)
 */
export function generateSubagentConfigs(settings: AgentModelSettings): Record<string, {
    name: string;
    description: string;
    model: string;
    tools: string[];
    ...
    for (const config of settings.scenarioConfigs) {
        ...
        const name = config.scenario === 'custom'
            ? config.customName || 'custom-agent'
            : `${config.scenario}-agent`;

        const description = config.scenario === 'custom'
