...
class AgentStore {
	...
	hydrateTasks(tasks: AgentTask[]) {
		...
		// 与运行时一致：taskHistory 仅保留最近 20 条，避免长会话下数组无限增长 + .find() O(n) 慢
		const trimmed = sorted.length > 20 ? sorted.slice(-20) : sorted;
...
// 模块级 stable actions — 引用永不变化，保证子组件 memo 正常工作
// 必须在 agentStore 单例之后定义
export const agentActions = {
	...
	setPendingErrorRecovery: agentStore.setPendingErrorRecovery.bind(agentStore),
	clearPendingErrorRecovery:
		agentStore.clearPendingErrorRecovery.bind(agentStore),
	getPendingErrorRecovery: agentStore.getPendingErrorRecovery.bind(agentStore),
