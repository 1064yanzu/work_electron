...
function ToolCallInlineImpl({
	...
	// 精确 selector：只订阅本组件对应的工具调用数据，避免其他 agent 状态变化触发无效重渲染
	const storeToolCall = useAgentStoreSelector((state) => {
		const task =
			state.currentTask?.id === taskId
				? state.currentTask
				: state.taskHistory.find((t) => t.id === taskId);
		return task?.toolCalls.find((tc) => tc.id === toolCallId) ?? null;
	});
...
export default memo(ToolCallInlineImpl);
