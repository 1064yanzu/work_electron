...
export function ChatHistory({
	...
	return (
		<div className="flex flex-col h-full bg-surface animate-in slide-in-from-left-5 duration-150">
			{/* Header Area */}
			<div className="flex flex-col gap-4 p-4 pb-2">
					...
					<div className="flex items-center gap-2">
						<Tooltip content="新建对话" placement="bottom">
							<button
								...
								aria-label="新建对话"
								className="p-2 text-text-muted hover:text-text-primary hover:bg-warm-200 rounded-xl transition-[color,background-color,border-color,opacity,box-shadow,transform] active:scale-95"
							>
						...
						<Tooltip content="关闭" placement="bottom">
							<button
								onClick={onClose}
								aria-label="关闭历史记录"
								className="p-2 text-text-light hover:text-text-secondary dark:hover:text-text-light hover:bg-warm-200 rounded-xl transition-[color,background-color,border-color,opacity,box-shadow,transform] active:scale-95"
							>
								<X className="w-5 h-5" />
							</button>
						</Tooltip>
				...
				<div className="relative group">
					...
					<input
						...
						className="w-full pl-9 pr-4 py-2.5 bg-warm-50/50 border border-transparent focus:border-border dark:focus:border-dark-border rounded-xl text-sm text-text-primary placeholder:text-text-light focus:outline-none focus:ring-2 focus:ring-cream-900/5 transition-[color,background-color,border-color,opacity,box-shadow,transform]"
					/>
				</div>
			</div>
...
function SessionItem({
	...
	return (
		<div
			...
			className={`
                        group relative flex items-start gap-3 px-3 py-3 rounded-xl cursor-pointer transition-[color,background-color,border-color,opacity,box-shadow,transform] duration-150
                        ${
													active
														? "bg-warm-200 shadow-sm ring-1 ring-cream-200/50 dark:ring-cream-700/50"
		...
		>
			...
			<div className="flex-1 min-w-0 pr-6">
				...
				>
					{session.title || "新对话"}
					{pinned ? (
						<span className="ml-2 text-[11px] text-text-secondary">置顶</span>
					) : null}
				</div>
			...
			<Tooltip content="删除对话" placement="left">
				<button
					...
					className="absolute right-2 top-2 p-1.5 opacity-0 group-hover:opacity-100 text-text-light hover:text-error hover:bg-[rgba(181,51,51,0.08)] rounded-lg transition-[color,background-color,border-color,opacity,box-shadow,transform] duration-150"
				>
