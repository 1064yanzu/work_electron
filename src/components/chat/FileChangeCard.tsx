...
export function FileChangeCard({ update }: { update: FileUpdate }) {
	...
	return (
		<div
			className={`group relative px-3 py-2.5 transition-all duration-200 hover:bg-warm-50/45 dark:hover:bg-zinc-900/35 ${
				isError
		...
		>
			<div className="flex items-start justify-between gap-4">
				<div className="flex items-center gap-3 min-w-0 flex-1">
					<div
						className={`relative w-7 h-7 rounded-md flex items-center justify-center shrink-0 ring-1 transition-all duration-200 ${
							isError
				...
				<div className="flex items-center gap-2 shrink-0 self-center">
					...
					{isRunning ? (
						...
						<Check className="w-4 h-4 text-emerald-600 dark:text-emerald-300 animate-in zoom-in-50 duration-200" />
					)}
				</div>
			</div>
