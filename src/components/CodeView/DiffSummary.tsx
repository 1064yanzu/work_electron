			...
			{isPending ? (
				<div className="flex items-center gap-1 flex-shrink-0">
					...
					<button
						...
						className="p-1 rounded text-zinc-400 hover:text-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-colors"
						title="接受"
					>
...
function DiffSummaryInner({ rootPath }: DiffSummaryProps) {
	...
	return (
		<div className="my-3 rounded-xl ring-1 ring-zinc-200/60 dark:ring-zinc-700/60 bg-white dark:bg-zinc-900 overflow-hidden">
			{/* 汇总头部 */}
			<button
				type="button"
				onClick={() => diffStore.toggleSummary()}
				className="w-full flex items-center justify-between gap-3 px-3 py-2.5 hover:bg-zinc-50/50 dark:hover:bg-zinc-800/30 transition-colors"
			>
			...
			{summaryExpanded && (
				<div className="border-t border-zinc-100 dark:border-zinc-800">
					...
					{/* 批量操作 */}
					{summaryStats.pending > 0 && (
						<div className="flex items-center justify-end gap-2 px-3 py-2 border-t border-zinc-100 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-800/20">
							<button
								type="button"
								onClick={() => rejectAllDiffs()}
								className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-zinc-500 dark:text-zinc-400 hover:bg-zinc-200/60 dark:hover:bg-zinc-700/60 transition-colors"
							>
								<X className="w-3 h-3" />
								全部拒绝
							</button>
