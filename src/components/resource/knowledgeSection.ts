/**
 * 左栏「知识」区：资料库 / 卡片 / Wiki 三个视图合成一个 rail 入口。
 *
 * 背景：活动栏平铺 6 个入口时，「资料库 / 卡片 / Wiki」三个其实是同一类东西
 * （沉淀下来的知识），各占一格既拉长了 rail，也没表达出它们的亲缘关系。
 * 合并后 rail 只剩 4 个入口，三者改由面板顶部的一条 pill tab 切换。
 *
 * `leftSidebarView` 的取值**保持不变**（仍是 "sources" / "cards" / "wiki"），
 * 所以所有依赖这三个值的地方（命令面板、ResourceSidebar 分支、深链）零改动。
 */
