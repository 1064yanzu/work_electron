...
import {
	...
	getPreviewPathLabel,
	splitPreviewLines,
} from "./filePreview/filePreviewUtils";
...
export function CodingFilePreviewPanel() {
	...
	const preview = useCodingWorkspaceSelector(
		(state) => state.selectedFilePreview,
	);
	...
	const lines = useMemo(
		() => splitPreviewLines(preview.content),
		[preview.content],
	);
						...
						{lines.map((line, lineIdx) => {
							...
							return (
								...
								>
									...
									<pre className="overflow-x-auto px-3 py-1.5 text-[12px] leading-6">
										<code>
											{lineTokens ? (
