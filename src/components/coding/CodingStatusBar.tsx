...
import { GitBranch, Shield, Cpu, Code2, Coins } from "lucide-react";
...
export function CodingStatusBar() {
	...
	const approvalLabel = useMemo(() => {
		...
		return (
			APPROVAL_LABELS[activeThread.approvalMode] || activeThread.approvalMode
		);
	}, [activeThread]);

	// 修复：gitStatus.branch 是顶层属性，不嵌套在 .status 下
	const branch = gitStatus?.branch ?? null;
	const backendLabel = activeThread
		? BACKEND_LABELS[activeThread.backend] || activeThread.backend
		: null;
	const modeLabel = activeThread
		? MODE_LABELS[activeThread.codingMode] || activeThread.codingMode
		: null;
