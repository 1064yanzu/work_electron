...
function flattenTree(nodes: FileTreeNode[], projectPath: string, prefix = ''): FlatFile[] {
	...
	for (let ti = 0; ti < t.length && qi < q.length; ti++) {
		if (t[ti] === q[qi]) {
			...
			if (ti > 0 && /[/\\._\-]/.test(t[ti - 1])) {
				score -= 1;
			} else if (ti > 0 && t[ti - 1] === t[ti - 1].toLowerCase() && target[ti] === target[ti].toUpperCase()) {
				score -= 1; // 驼峰边界
			}
			lastPos = ti;
			qi++;
...
function QuickFileSearchModalInner({
	...
	const filteredFiles = useMemo<ScoredFile[]>(() => {
		...
		for (const file of allFiles) {
			...
			const best = nameScore === -1 && pathScore === -1
				? -1
				: nameScore === -1 ? pathScore
				: pathScore === -1 ? nameScore
