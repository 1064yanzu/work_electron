		...
		<>
			...
			>
				<div className="flex items-center gap-2 min-w-0">
					{selectedOption?.icon && (
						<selectedOption.icon className="w-4 h-4 text-text-muted" />
					)}
					<div className="flex flex-col items-start truncate">
						{label && (
							<span className="text-[10px] text-text-light font-medium uppercase tracking-wider mb-0.5">
								{label}
							</span>
						)}
						<span
							className={cn(
								...
								value ? "text-text-primary" : "text-text-light",
							)}
						>
				...
				<ChevronDown
					className={cn(
						"w-4 h-4 text-text-light group-hover:text-text-muted dark:group-hover:text-text-light transition-transform duration-200",
						isOpen ? "rotate-180" : "",
					)}
				/>
			</button>
							...
							{groups ? (
								groups.map((group) => (
									<div key={group.label} className="mb-1 last:mb-0">
										<div className="px-3 py-1.5 text-xs font-semibold text-text-muted bg-warm-50/60 rounded-lg mb-1">
								...
								<div className="px-3 py-8 text-center text-xs text-text-light">
									暂无选项
								</div>
							) : (
