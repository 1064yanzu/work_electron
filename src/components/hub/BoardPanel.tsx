		...
		<div className="flex flex-col h-full min-h-0">
			...
			<div className="px-5 pt-4 pb-3 shrink-0 space-y-2.5">
				<div className="flex items-start gap-1.5 text-[10.5px] text-text-light leading-relaxed">
					<FileText className="w-3 h-3 mt-[1px] shrink-0" strokeWidth={1.6} />
					...
					<div className="flex items-center gap-0.5 shrink-0">
						{KIND_ORDER.map((kind) => (
							<button
								...
								className={cn(
									"px-2 py-1 rounded-md text-[10.5px] transition duration-200",
									draftKind === kind
				...
				<div className="flex items-center gap-1.5">
					<input
						...
						className="flex-1 px-3 py-2 text-[12px] bg-surface dark:bg-cream-900/40 border border-border rounded-lg text-text-secondary placeholder:text-text-light focus:outline-none focus:border-primary/40 focus:ring-2 focus:ring-primary/8 transition duration-200"
					/>
					<button
						type="button"
						onClick={() => void submit()}
						disabled={!draft.trim()}
						className="p-2 rounded-lg bg-primary text-primary-foreground hover:bg-primary-hover transition duration-200 disabled:opacity-40 disabled:cursor-not-allowed"
						title="写入白板"
					>
			...
			<div className="flex-1 overflow-y-auto scrollbar-hide px-5 pb-8 space-y-4">
				{loading && entries.length === 0 && (
					<p className="text-[11px] text-text-light py-8 text-center">
						正在读取白板…
					</p>
				)}
				...
				{KIND_ORDER.map((kind) => {
					...
					return (
						<section key={kind}>
							<h4 className="text-[10px] font-medium tracking-wide text-text-light uppercase mb-1.5">
								{BOARD_KIND_LABEL[kind]}
							</h4>
							<ul className="space-y-px">
								{list.map((entry) => (
									<li
										key={entry.id}
										className="group flex items-start gap-2 px-2.5 py-2 rounded-lg hover:bg-warm-200/40 dark:hover:bg-cream-800/30 transition duration-200"
									>
										{kind === "next" && (
											<button
												type="button"
												...
												className={cn(
													"mt-[2px] w-3.5 h-3.5 rounded border shrink-0 flex items-center justify-center transition duration-200",
													entry.state === "done"
										...
										<button
											...
											className="opacity-0 group-hover:opacity-100 p-1 rounded text-text-light hover:text-error hover:bg-error/8 transition duration-200 shrink-0"
										>
